    ...
    <AppShell>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Card>
          ...
          <div className="space-y-2">{suppliers.map((s) => <div key={s.id} className="rounded-md bg-slate-50 p-3"><p className="font-bold">{s.name}</p><p className="text-sm text-slate-500">{s.phone || "No phone"} {s.email ? `- ${s.email}` : ""}</p></div>)}</div>
        </Card>
      </div>
    </AppShell>
  );
